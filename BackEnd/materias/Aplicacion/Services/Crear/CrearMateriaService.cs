...
namespace BackEnd.materias.Aplicacion.Services.Crear
{
    public class CrearMateriaService
    {
        ...
        {
            var materia = _unitOfWork.MateriaServiceRepository.FindFirstOrDefault(t => t.IdDocente == request.IdDocente);
            if (materia == null)
            {
                Dominio.Entidades.Materias newMateria = new Dominio.Entidades.Materias(request.idMateria, request.NombreMateria, request.IdDocente, request.IdCurso);
